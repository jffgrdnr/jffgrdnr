...
class ControllerModuleKbm extends Controller
{
	...
	public function comment()
	{
		...
		if ($enable_captcha)
		{
			$this->data['captcha_url'] = 'index.php?route=tool/captcha';
		}
		...
		if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/module/kbm_comment.tpl')) {
			echo $this->load->view($this->config->get('config_template') . '/template/module/kbm_comment.tpl', $this->data);
		} else {
			echo $this->load->view('default/template/module/kbm_comment.tpl', $this->data);
		}
	}
	...
	public function writeComment()
	{
		...
		try
		{
			...
			if ($this->model->enableCommentCaptcha($customer_id, $customer_group_id) && $input['captcha'] != $this->data['captcha'])
			{
    ...
    private function renderView($template)
    {
	    ...
	    if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/' . $template)) {
		    $this->response->setOutput($this->load->view($this->config->get('config_template') . '/template/' . $template, $this->data));
	    } else {
		    $this->response->setOutput($this->load->view('default/template/' . $template, $this->data));
	    }
    }
